...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (explicit-shell-file-name
     (or (and (file-exists-p "/opt/zerobrew/bin/zsh") "/opt/zerobrew/bin/zsh")
         (and (file-exists-p "/opt/homebrew/bin/zsh") "/opt/homebrew/bin/zsh")
         (executable-find "zsh")
         "/bin/sh"))
    (line-spacing 0.3)
...
[[https://tree-sitter.github.io/tree-sitter/][Tree-sitter]]: Emacs 31 ships pinned grammar recipes with each built-in
ts-mode and prompts before installing a missing grammar
(~treesit-auto-install-grammar~ defaults to ~ask~), so only grammars for
third-party modes need declaring here.
...
Emacs 31's ~treesit-enabled-modes~ replaces the old hand-maintained
~major-mode-remap-alist~ / ~auto-mode-alist~ dance: setting it to ~t~
remaps every base mode to its tree-sitter variant, and the built-in
ts-modes register their own file associations.  It must be set with
~setopt~ (not ~setq~) because the remapping happens in the option's
custom setter.  Only file extensions Emacs doesn't know about still
need adding to ~auto-mode-alist~ manually.

#+begin_src emacs-lisp
  (setopt treesit-enabled-modes t)

  (dolist (entry '(("\\.lua\\'"         . lua-ts-mode)
                   ("\\.jsonc\\'"       . json-ts-mode)
                   ("\\.json5\\'"       . json-ts-mode)
                   ("\\.webmanifest\\'" . json-ts-mode)
                   ("\\.sls\\'"         . yaml-ts-mode)))
...
[[https://codeberg.org/akib/emacs-eat][Eat]] (Emulate A Terminal) is a fast terminal emulator written in pure
Elisp. Also integrates with eshell to handle visual commands.
...
[[https://github.com/xenodium/agent-shell][agent-shell]] - A native Emacs buffer for interacting with LLM agents
...
#+begin_src emacs-lisp
  (use-package dashboard
    :config
    ;; Elpaca processes package queues asynchronously after init, so we
    ;; need these hooks to ensure the dashboard is built after all
    ...
    ;; Display dashboard in new frames when using emacs daemon / emacsclient.
...
#+begin_src emacs-lisp
  (use-package eglot
    ...
    :config
    ;; mhtml-ts-mode lists js-mode among its parents, which matches
    ;; eglot's JS/TS server entry before its HTML one -- pin it.
...
#+begin_src emacs-lisp
  (use-package flycheck
    ...
    :config
    ;; Report Eglot's LSP diagnostics through Flycheck rather than Flymake.
    (global-flycheck-eglot-mode 1)

    ;; Show diagnostics inline, next to the code (Error Lens style).
    (global-flycheck-annotate-mode 1)

    ;; Disable Flycheck's default prose checker so Vale can take over.
    (setq-default flycheck-disabled-checkers
                  (append flycheck-disabled-checkers
                          '(proselint)))
    (flycheck-define-checker vale
      "Lint prose with Vale (https://vale.sh)."
      :command ("vale" "--output" "line" source)
      :error-patterns
      ((error line-start (file-name) ":" line ":" column ":" (id (one-or-more (not (any ":")))) ":" (message) line-end))
      :modes (gfm-mode markdown-mode org-mode rst-mode text-mode))
    (add-to-list 'flycheck-checkers 'vale 'append)

    ;; Vale only lints prose, so chain to the Eglot bridge to also get the
    ;; LSP server's diagnostics (e.g. marksman's in Markdown).  Inert in
    ;; prose buffers Eglot does not manage.
    (flycheck-add-next-checker 'vale 'eglot-check)

    (defun rt/flycheck-prefer-vale ()
      "Prefer Vale by default in prose-oriented buffers."
      (when (and (derived-mode-p 'gfm-mode 'markdown-mode 'org-mode 'rst-mode 'text-mode)
                 (flycheck-may-use-checker 'vale))
        (setq-local flycheck-checker 'vale)))

    (add-hook 'flycheck-mode-hook #'rt/flycheck-prefer-vale))
#+end_src

** GCMH

[[https://github.com/emacsmirror/gcmh][GCMH]] (Garbage Collector Magic Hack) enforces a sneaky-gcmh strategy for
garbage collection: use a large GC threshold during normal editing to
avoid pauses, then collect when Emacs is idle. Works in tandem with the
startup optimisation in ~early-init.el~ which sets the threshold to max
during init.
...
Built-in ~project.el~ configuration. Use ~project-vc-extra-root-markers~
to recognise projects by common marker files beyond just ~.git~.

#+begin_src emacs-lisp
  (use-package project
    :ensure nil
    :custom
    ;; Jump straight to find-file when switching projects.
    (project-switch-commands #'project-find-file)
    :config
    (setq project-vc-extra-root-markers
          '(".project" ".projectile" "Cargo.toml" "go.mod"
...
[[https://protesilaos.com/emacs/pulsar][Pulsar]] - Pulse highlight on demand or after running select functions.
...
[[https://github.com/minad/jinx][Jinx]] - a fast just-in-time spell-checker that checks all visible text
using the [[https://rrthomas.github.io/enchant/][Enchant]] library. Enchant uses aspell as its English backend so
my existing personal dictionary ~.aspell.en.pws~ is still read; new
words saved when correcting go to ~.config/enchant/en_GB-ise.dic~.
Requires ~enchant~ and ~pkgconf~ (Homebrew) to compile its native
module. In programming modes only comments, strings, and docs are
checked.

Use ~C-.~ or ~M-$~ for correcting spelling, ~C-u M-$~ corrects the
whole buffer.
...
Workspace management using built-in ~tab-bar-mode~. The tab bar is hidden
visually — workspaces are displayed in the echo area and managed entirely
through keybindings. See [[file:elisp/tab-workspaces.el][elisp/tab-workspaces.el]] for the implementation.

| Binding     | Action                           |
|-------------+----------------------------------|
| ~C-x t t~   | Show workspaces in echo area     |
| ~C-x t N~   | New blank workspace              |
| ~C-x t P~   | Switch to project workspace      |
| ~C-x t b~   | Switch buffer (workspace-scoped) |
| ~C-x t K~   | Kill all buffers in workspace    |
| ~C-x t 1-9~ | Jump to workspace by number      |
...
#+begin_src emacs-lisp
  (use-package tab-bar
    ...
    (setq tab-bar-show nil
          tab-bar-new-tab-choice #'rt-workspace--fallback-buffer)
    (tab-bar-mode 1)
    (require 'tab-workspaces)
    ;; Ensure emacsclient frames never show the tab bar.  tab-bar
    ;; updates default-frame-alist dynamically as tabs are added, so
    ;; we must override the frame parameter explicitly on creation.
    ...
    :bind (:map tab-prefix-map
                ("t"   . rt-workspace-display)
                ("N"   . rt-workspace-new)
                ("P"   . rt-workspace-switch-to-project)
                ("b"   . rt-workspace-switch-buffer)
                ("K"   . rt-workspace-kill-all-buffers)
                ("1"   . rt-workspace-switch-to-1)
                ("2"   . rt-workspace-switch-to-2)
                ("3"   . rt-workspace-switch-to-3)
                ("4"   . rt-workspace-switch-to-4)
                ("5"   . rt-workspace-switch-to-5)
                ("6"   . rt-workspace-switch-to-6)
                ("7"   . rt-workspace-switch-to-7)
                ("8"   . rt-workspace-switch-to-8)
                ("9"   . rt-workspace-switch-to-9)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package org-contacts
    :ensure (org-contacts :host github :repo "emacsmirror/org-contacts" :main "org-contacts.el")
...
[[https://codeberg.org/meow_king/typst-ts-mode][typst-ts-mode]] - Tree-sitter based major mode for editing Typst files.
...
[[https://github.com/havarddj/typst-preview.el][typst-preview]] - Live preview of Typst files using tinymist.

#+begin_src emacs-lisp
  (use-package typst-preview
    :ensure (typst-preview :host github :repo "havarddj/typst-preview.el")
    :after typst-ts-mode
    :ensure-system-package
...
#+begin_src emacs-lisp
  (use-package mu4e
    :ensure nil
    :load-path "/opt/zerobrew/share/emacs/site-lisp/mu/mu4e"
...
#+begin_src emacs-lisp
  ...
  - Automation and repeatability")))
    ...
    :config
    ;; 'gptel-backend' must not go through ':custom'.  use-package stores the
    ;; value form unevaluated as a custom theme value, and gptel-request.el's
    ;; own '(defcustom gptel-backend ...)' then evaluates it mid-load, where
    ;; the 'gptel-make-anthropic' autoload requires gptel-request again --
    ;; a recursive require that breaks 'M-x gptel' outright.
    (setq gptel-backend (gptel-make-anthropic "Claude"
                          :stream t
                          :key #'rt/get-anthropic-key))

    ;; Move cursor after response.  'gptel-post-response-functions' has no
    ;; '-hook' suffix, so ':hook' would target a non-existent
    ;; 'gptel-post-response-functions-hook' and never run.
...
#+begin_src emacs-lisp
  (use-package mcp
    ...
    ;; Start the servers when gptel loads rather than on a startup hook: a
    ;; startup hook can never fire here, as ':after gptel' defers the
    ;; add-hook until gptel loads, which is after the hook has already run.
    :config
    (require 'mcp-hub)
