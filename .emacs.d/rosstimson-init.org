...
Start pinentry so that GPG PIN can be entered via the minibuffer, for
example when signing Git commits within Magit.
...
This is another thing needed for Emacs to prompt for GPG PIN within
the minibuffer, there also needs to be a setting enabled in
