...
- **Global scope**: `$HOME/.config/opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `$HOME/.config/opencode/notes/`)
...
2. Glob `$HOME/.config/opencode/notes/*.md` (if directory exists) — global notes
...
Global ($HOME/.config/opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `$HOME/.config/opencode/notes/` — works in any directory
