...
**Project skills:** @$HOME/.config/opencode/get-shit-done/references/project-skills-discovery.md
...
@$HOME/.config/opencode/get-shit-done/references/planner-source-audit.md for constraint examples.
...
**Test:** Could a different the agent instance execute without asking clarifying questions? If not, add specificity. See @$HOME/.config/opencode/get-shit-done/references/planner-antipatterns.md for vague-vs-specific comparison table.

## TDD Detection

**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively — all eligible tasks MUST use `type: tdd`. Read @$HOME/.config/opencode/get-shit-done/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.
...
@$HOME/.config/opencode/get-shit-done/references/planner-antipatterns.md
...
```bash
node "$HOME/.config/opencode/get-shit-done/bin/gsd-tools.cjs" graphify query "<phase-goal-keyword>" --budget 2000
```
